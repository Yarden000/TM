...
class MapGenerator:
    ...
    def __init__(self, pixel_sise, base_gris_size = 4, octaves = 1, persistence = 0.5, frequency = 2):
        self.pixel_size = pixel_sise
        self.base_gris_size = base_gris_size
    ...
    def perlin_noise_simple(self):
        ...
        for p in range(self.octaves):
            ...
            self.cell_number = int(max(HEIGHT, WIDTH) / self.pixel_size) # has to be a multiple of the grid_size
            self.value_grid = {}
    ...
    def display(self): #test
        ...
        for x in range(self.cell_number):
            for y in range(self.cell_number):
                if self.total_value_grid[(x, y)] > 0.01:   #0:#
                    self.color = (0, round((self.total_value_grid[(x, y)] * 255) / self.extreme), 0)
                elif self.total_value_grid[(x, y)] < -0.01:
                    self.color = (round(abs(self.total_value_grid[(x, y)] * 255) / self.extreme), 0, 0)
                else:
